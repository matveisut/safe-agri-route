...
- **Опционально (§10, эвристики):** признаки по MAVLink → fusion → при регистрации контекста миссии возможна кандидатная зона jammer и авто-replan (`telemetry_features`, `threat_fusion`, `mission_fusion_runtime`; см. `diplom/ТЗ.md` §10 и `diplom/Промпты.md` 9–12).
...
```bash
# Однократно: создать venv с pymavlink/MAVProxy и зависимостями sim_vehicle.py
python3 -m venv ~/venv-ardupilot
source ~/venv-ardupilot/bin/activate
pip install pymavlink MAVProxy pexpect
...
> Остановка: `bash start_sitl_wsl.sh stop` (освобождает порты 14550–14580) или вручную: `screen -ls | grep sitl | awk '{print $1}' | xargs -I{} screen -S {} -X quit`
