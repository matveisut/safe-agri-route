...
> Актуализация 22.04.2026: в runtime-контур добавлены state machine детектора, dynamic/suspected zones, controlled replan, а также `PLR` (Packet Loss Rate) в `jam_prob`.
...
### 4.3 Дипломная симуляция (`backend/simulation/`)

Для сравнения с baseline в §1.5 используется **отдельная** метрика «покрытие у трека»: доля ячеек дискретной сетки поля, чей центр находится в радиусе от полилинии маршрута (см. `simulation/metrics.py`). Она **не совпадает** с `estimated_coverage_pct` из `plan_mission` (достижимые точки сетки риска). В сценарии 3 по этой метрике покрытие baseline может **быстро насыщаться** (змейка по полосам); в тексте ВКР сравнивать динамику и итог SAR после перепланирования, а не жёсткие пороги из устаревших формулировок промпта.
...
## 7. Автоматическое обновление риска из телеметрии (§10, MVP)

**Статус в коде:** реализован **эвристический** контур (без ML и без отдельных драйверов SDR/«железного» IMU) — модули `telemetry_features.py`, `threat_fusion.py`, `mission_fusion_runtime.py`; см. `Промпты.md` 9–12 и `knowledge/architecture.md`.

По потоку **MAVLink** вычисляются скользящие признаки стабильности (GNSS, канал, IMU-прокси по траектории, согласованность с роем, а также **PLR / Packet Loss Rate**), затем **fusion** даёт скаляр угрозы ∈ [0, 1]. Для PLR используется формула `plr = lost_packets / max(total_packets, 1)` (или fallback-оценка по пропускам интервалов телеметрии, если счётчики недоступны). При устойчивом превышении порога и после debounce формируется **круговая зона jammer** вокруг текущей точки и вызывается **`replan_on_new_risk_zone`** (с ограничением частоты). Клиент может зарегистрировать миссию через **`POST /api/v1/mission/{id}/fusion-context`**.

**Перспектива (полная §10 в `ТЗ.md`):** расширенные отчёты GNSS/IMU, SDR, внешние слои — пока **не** в репозитории; текст ВКР может ссылаться на целевую модель отдельно от MVP-реализации.
